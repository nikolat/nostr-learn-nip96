...
<fieldset class="tab-content" id="field-delete">
  ...
  <dl>
    ...
    <dt>
      <label for="file-hash-to-delete"
        >The SHA-256 hash of the original file</label
      >
    </dt>
    <dd>
      <input
    ...
    <dd>
      <button
        id="delete"
        on:click={deleteFileExec}
        disabled={!fileHashToDelete || isInProcess}>Delete</button
      >
    </dd>
    <dt>Result</dt>
    <dd>
      <details>
        ...
        <pre><code
            >{JSON.stringify(fileDeleteResponse, undefined, 2) ?? ''}</code
          ></pre>
      </details>
    </dd>
  </dl>

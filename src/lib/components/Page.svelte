...
const getServerList = async (): Promise<string[]> => {
	...
	const ev10096: NostrEvent | undefined = await fetcher.fetchLastEvent(
		Array.from(relaySet),
		{ kinds: [10096], authors: [targetPubkey] },
	);
	...
	for (const tag of ev10096.tags.filter(tag => tag.length >= 2 && tag[0] === 'server' && URL.canParse(tag[1]))) {
...
<main>
	<fieldset id="field-settings">
		...
		<button id="get-settings" on:click={getNpubWithNIP07}>NIP-07</button>
		<label for="get-settings">for the kind 10096 event</label>
		<input id="npub" type="text" placeholder="npub1... or nprofile1..." bind:value={npub} on:change={setUploaderURLs} />
	</fieldset>
...
:global(#tab-upload:checked ~ #field-upload,
...
:global(fieldset input[type="text"]), #npub {
